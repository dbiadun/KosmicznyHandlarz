function createStarshipOnPlanetPopupItemsToSellListElement(item: ItemOnStarship, form: HTMLFormElement, planet: Planet, starship: Starship): HTMLLIElement {
...
function addStarshipOnPlanetPopupItemsToSellListElement(itemsList: HTMLUListElement, item: ItemOnStarship, planet: Planet, starship: Starship) {
  let form: HTMLFormElement = itemsList.parentNode as HTMLFormElement;
  let element: HTMLElement = createStarshipOnPlanetPopupItemsToSellListElement(item, form, planet, starship);
  itemsList.appendChild(element);
}

function updateStarshipOnPlanetItemsToSellList(list: HTMLUListElement, items: ItemOnStarship[], planet: Planet, starship: Starship) {
...
function createStarshipOnPlanetPopupItemsToBuyListElement(item: ItemOnPlanet, form: HTMLFormElement, planet: Planet, starship: Starship): HTMLLIElement {
  let nameSpan = document.createElement("span");
  nameSpan.innerText = item.getItemName();
  nameSpan.classList.add("aligner");
...
function addStarshipOnPlanetPopupItemsToBuyListElement(itemsList: HTMLUListElement, item: ItemOnPlanet, planet: Planet, starship: Starship) {
  let form: HTMLFormElement = itemsList.parentNode as HTMLFormElement;
  let element: HTMLElement = createStarshipOnPlanetPopupItemsToBuyListElement(item, form, planet, starship);
  itemsList.appendChild(element);
}

function updateStarshipOnPlanetItemsToBuyList(list: HTMLUListElement, items: ItemOnPlanet[], planet: Planet, starship: Starship) {
  while (list.childElementCount > 1) {
    list.removeChild(list.lastElementChild as HTMLElement);
  }
  for (let item of items) {
